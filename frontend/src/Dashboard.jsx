  ...
  const processApps = (tpoLogs) => {
    ...
    tpoLogs.forEach(row => {
      ...
      const log = {
        ...
        course: getVal('course'), status: getVal('status') || 'Applied', date: getVal('dateplaced') || getVal('timestamp'),
    ...
    uniqueApps.forEach(app => {
      ...
      if (isPlaced) pPl++;
      if (isPlaced) { let c = getStandardDomain(app.course); domCount[c] = (domCount[c] || 0) + 1; }
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (statsRes.data.success) { setStats(statsRes.data.stats); setEvents(statsRes.data.events || []); }
        if (reportsRes.data.success) {
          processApps(reportsRes.data.tpoLogs || []);
          setActiveIssues((reportsRes.data.issues || []).filter(i => (i.status||'').toLowerCase() !== 'resolved').slice(0, 3));
        }
          ...
          <div className="bento-card profile-bento">
            <div className="profile-img-container">
              {tpoData?.photo ? (
                ...
                <div className="profile-fallback">{tpoData?.name?.charAt(0) || 'U'}</div>
              )}
            </div>
              ...
              {recentPlacements.length > 0 ? recentPlacements.map((p, i) => (
                <div key={i} className="clean-row">
                  <div className="cl-left">
                    <div className="cl-avatar">{p.name.charAt(0)}</div>
                  ...
                  <div className="cl-right">
                    ...
                    <div className="cl-sub status-pill green">{(p.status||'Placed').toUpperCase()}</div>
                  </div>
                </div>
