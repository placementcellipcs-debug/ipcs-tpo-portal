...
export default function Settings() {
  ...
  return (
    <Layout>
      <div className="page-container" style={{ padding: '0 20px', maxWidth: '1000px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '30px' }}>
          ...
          <div style={{ flex: '1 1 250px', maxWidth: '300px' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              <button 
                ...
                style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '12px 20px', background: activeTab === 'profile' ? 'rgba(56, 189, 248, 0.1)' : 'var(--card-bg)', color: activeTab === 'profile' ? '#38bdf8' : 'var(--text-muted)', border: '1px solid', borderColor: activeTab === 'profile' ? 'rgba(56, 189, 248, 0.3)' : 'transparent', borderLeft: activeTab === 'profile' ? '3px solid #38bdf8' : '3px solid transparent', borderRadius: '8px', cursor: 'pointer', textAlign: 'left', fontWeight: activeTab === 'profile' ? 'bold' : 'normal', transition: '0.2s' }}
              >
                <User size={20} weight={activeTab === 'profile' ? "fill" : "regular"} /> Account Profile
              </button>
              <button 
                onClick={() => setActiveTab('security')}
                style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '12px 20px', background: activeTab === 'security' ? 'rgba(56, 189, 248, 0.1)' : 'var(--card-bg)', color: activeTab === 'security' ? '#38bdf8' : 'var(--text-muted)', border: '1px solid', borderColor: activeTab === 'security' ? 'rgba(56, 189, 248, 0.3)' : 'transparent', borderLeft: activeTab === 'security' ? '3px solid #38bdf8' : '3px solid transparent', borderRadius: '8px', cursor: 'pointer', textAlign: 'left', fontWeight: activeTab === 'security' ? 'bold' : 'normal', transition: '0.2s' }}
              >
          ...
          <div style={{ flex: '2 1 500px', background: 'var(--card-bg)', border: '1px solid var(--card-border)', borderRadius: '16px', padding: '30px' }}>
            ...
            {activeTab === 'profile' && (
              <div>
                <h2 style={{ margin: '0 0 20px 0', borderBottom: '1px solid var(--card-border)', paddingBottom: '15px', color: '#fff' }}>Profile Information</h2>
                <div style={{ display: 'grid', gap: '20px' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                    ...
                    <div>
                      ...
                      <div style={{ background: 'var(--bg-dark)', padding: '12px', borderRadius: '8px', color: '#fff' }}>{tpoData.assignedCourse || 'All Courses'}</div>
                    </div>
                  </div>
                  <div>
                    <label style={{ display: 'block', fontSize: '0.85rem', color: 'var(--text-muted)', marginBottom: '5px', fontWeight: 'bold' }}>ASSIGNED BRANCHES</label>
                    <div style={{ background: 'var(--bg-dark)', padding: '12px', borderRadius: '8px', color: '#fff' }}>{(tpoData.assignedBranchesArray || []).join(', ')}</div>
                  </div>
                </div>
              </div>
            )}
            ...
            {activeTab === 'security' && (
              <div>
                <h2 style={{ margin: '0 0 20px 0', borderBottom: '1px solid var(--card-border)', paddingBottom: '15px', color: '#fff' }}>Security & Password</h2>
