...
export default function Layout({ children }) {
  ...
  // 🚨 AUTO-REPAIR PROTOCOL: If session is broken, instantly force logout.
  if (!tpoData || !tpoData.email) {
    if (location.pathname !== '/') {
      localStorage.clear(); // Wipe bad cache
      window.location.href = '/'; // Hard redirect to login
    }
    return <div style={{ height: '100vh', background: '#0f172a' }}>{children}</div>;
  }
  ...
  }, [isBamOnly, location.pathname, navigate]);

  // LIVE NOTIFICATIONS ENGINE
  useEffect(() => {
    if (isBamOnly) {
      setNotifications([{ title: "System Active", desc: "Asset Management Console is online.", icon: <Barcode size={18} weight="bold" />, color: "#10b981", bg: "rgba(16, 185, 129, 0.1)", time: "Just now" }]);
      return;
    }
    try {
      ...
      if (logsStr) {
        ...
        const sorted = logs.sort((a,b) => new Date(b.TimeStamp || b.Timestamp || 0) - new Date(a.TimeStamp || a.Timestamp || 0)).slice(0, 5); 
        const mappedNotifs = sorted.map(log => ({
          title: "System Update", desc: "A record was updated in the system.", icon: <ListChecks size={18} weight="bold" />, color: "#38bdf8", bg: "rgba(56, 189, 248, 0.1)", time: "Recently"
        }));
        ...
        <header className="top-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '15px 30px' }}>
          ...
          <div className="header-actions" style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
            <div className="theme-picker" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <button onClick={toggleTheme} style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: 'var(--text-muted)' }}>{theme === 'dark' ? '🌙' : '☀️'}</button>
            </div>
      ...
      <div className={`drawer-overlay ${isDrawerOpen ? 'open' : ''}`} onClick={(e) => { if(e.target.classList.contains('drawer-overlay')) setIsDrawerOpen(false); }}>
        <div className="drawer-card" style={{ backgroundImage: `linear-gradient(rgba(11, 17, 32, 0.85), rgba(11, 17, 32, 0.98)), url('https://lh3.googleusercontent.com/d/1dr27VR3Xu8EwDf4dCAO1ucq441VjpfwB')`, backgroundSize: 'cover', backgroundPosition: 'center', borderLeft: '1px solid rgba(255,255,255,0.1)', display: 'flex', flexDirection: 'column', height: '100%' }}>
          ...
          <div className="drawer-header" style={{ padding: '30px 20px', position: 'relative', borderBottom: '1px solid rgba(255,255,255,0.1)', flexShrink: 0 }}>
            <div className="drawer-close-btn" onClick={() => setIsDrawerOpen(false)} style={{ position: 'absolute', top: '15px', right: '15px', background: 'rgba(255,255,255,0.2)', color: '#fff', borderRadius: '50%', padding: '4px', cursor: 'pointer' }}><X size={16} weight="bold" /></div>
            <div className="drawer-profile-row" style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
              ...
              <div style={{ color: '#fff' }}>
                <strong style={{ display: 'block', fontSize: '1.25rem', fontWeight: 700 }}>{tpoData.name}</strong>
                <span style={{ fontSize: '0.75rem', opacity: 0.9, textTransform: 'uppercase', color: '#cbd5e1' }}>{tpoData.role || 'User'}</span>
              </div>
            </div>
          </div>
          ...
          <div className="drawer-menu" style={{ padding: '15px', flex: 1, overflowY: 'auto' }}>
            ...
            {!isBamOnly && (
              <>
                ...
                {showTracker && <div className="drawer-item" onClick={() => handleNav('/tracker')}><div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}><Files size={22} color={isActive('/tracker')} /> <span style={{ color: isActive('/tracker') === '#38bdf8' ? '#fff' : '#cbd5e1' }}>Job Tracker</span></div><span style={{ color: '#64748b' }}>›</span></div>}
                {showReports && <div className="drawer-item" onClick={() => handleNav('/reports')}><div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}><ChartBar size={22} color={isActive('/reports')} /> <span style={{ color: isActive('/reports') === '#38bdf8' ? '#fff' : '#cbd5e1' }}>Reports</span></div><span style={{ color: '#64748b' }}>›</span></div>}
                ...
                {showStudentApps && <div className="drawer-item" onClick={() => handleNav('/applications')}><div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}><ListChecks size={22} color={isActive('/applications')} /> <span style={{ color: isActive('/applications') === '#38bdf8' ? '#fff' : '#cbd5e1' }}>Student Apps</span></div><span style={{ color: '#64748b' }}>›</span></div>}
                ...
                {!isTrainer && <div className="drawer-item" onClick={() => handleNav('/placement-drives')}><div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}><IdentificationCard size={22} color={isActive('/placement-drives')} /> <span style={{ color: isActive('/placement-drives') === '#38bdf8' ? '#fff' : '#cbd5e1' }}>Placement Drives</span></div><span style={{ color: '#64748b' }}>›</span></div>}
                {!isTrainer && <div className="drawer-item" onClick={() => handleNav('/clients')}><div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}><Handshake size={22} color={isActive('/clients')} /> <span style={{ color: isActive('/clients') === '#38bdf8' ? '#fff' : '#cbd5e1' }}>Clients & Partners</span></div><span style={{ color: '#64748b' }}>›</span></div>}
                ...
                {showStudyMaterials && <div className="drawer-item" onClick={() => handleNav('/study-materials')}><div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}><Book size={22} color={isActive('/study-materials')} /> <span style={{ color: isActive('/study-materials') === '#38bdf8' ? '#fff' : '#cbd5e1' }}>Study Materials</span></div><span style={{ color: '#64748b' }}>›</span></div>}
                {showTrainerLogs && <div className="drawer-item" onClick={() => handleNav('/trainer-logs')}><div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}><Notebook size={22} color={isActive('/trainer-logs')} /> <span style={{ color: isActive('/trainer-logs') === '#38bdf8' ? '#fff' : '#cbd5e1' }}>Daily Log Report</span></div><span style={{ color: '#64748b' }}>›</span></div>}
                {!userRole.includes('MANAGER') && !isTpo && <div className="drawer-item" onClick={() => handleNav('/exams')}><div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}><FileText size={22} color={isActive('/exams')} /> <span style={{ color: isActive('/exams') === '#38bdf8' ? '#fff' : '#cbd5e1' }}>Exams Hub</span></div><span style={{ color: '#64748b' }}>›</span></div>}
              </>
            )}
            ...
            {(isSuperAdmin || userRole.includes('MANAGER') || isBamOnly) && (
              <div style={{ marginTop: isBamOnly ? '0' : '20px', paddingTop: isBamOnly ? '0' : '20px', borderTop: isBamOnly ? 'none' : '1px solid rgba(255,255,255,0.1)' }}>
                ...
                {isSuperAdmin && <div className="drawer-item" onClick={() => handleNav('/assets/settings')}><div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}><DesktopTower size={22} color={isActive('/assets/settings') === '#38bdf8' ? '#38bdf8' : '#94a3b8'} /> <span style={{ color: isActive('/assets/settings') === '#38bdf8' ? '#fff' : '#cbd5e1' }}>Asset Config</span></div><span style={{ color: '#64748b' }}>›</span></div>}
              </div>
            )}

            {showManageAdmin && !isBamOnly && (
               <div style={{ marginTop: '20px', paddingTop: '20px', borderTop: '1px solid rgba(255,255,255,0.1)' }}>
                 <span style={{ display: 'block', padding: '0 1.5rem', fontSize: '0.75rem', fontWeight: 800, color: '#64748b', textTransform: 'uppercase', letterSpacing: '1px', margin: '0 0 10px 0' }}>System Admin</span>
