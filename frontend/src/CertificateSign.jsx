...
export default function CertificateSign() {
  ...
  useEffect(() => {
    axios.get(`https://ipcs-tpo-portal.onrender.com/api/tpo/clients/${id}`)
      .then(res => {
        if (res.data.success) {
  ...
  const generateAndSubmitPDF = async () => {
    ...
    try {
      ...
      const res = await axios.post('https://ipcs-tpo-portal.onrender.com/api/tpo/clients/submit-mou', formData, { headers: { 'Content-Type': 'multipart/form-data' }});
