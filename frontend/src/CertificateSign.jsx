...
export default function CertificateSign() {
  ...
  useEffect(() => {
    axios.get(`http://localhost:5000/api/tpo/clients/${id}`)
      .then(res => {
        if (res.data.success) {
  ...
  const generateAndSubmitPDF = async () => {
    ...
    try {
      ...
      const res = await axios.post('http://localhost:5000/api/tpo/clients/submit-mou', formData, { headers: { 'Content-Type': 'multipart/form-data' }});
