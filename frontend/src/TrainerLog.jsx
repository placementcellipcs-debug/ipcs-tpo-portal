...
import { CircleNotch, Plus, FloppyDisk, PencilSimple, X, Checks, Notebook } from '@phosphor-icons/react';
...
export default function TrainerLog() {
  ...
  const isTL = upperRole.includes('TECHNICAL LEAD') || upperRole.includes('TTH') || upperRole.includes('RTH') || upperRole.includes('MANAGER') || tpoData?.accessType === 'superadmin';
  ...
  const fetchLogs = async () => {
    try {
      ...
      if (res.data.success) {
        ...
        // Restrict view for Trainers/TLs to their specific branch
        if (tpoData.accessType !== 'superadmin' && !tpoData.assignedBranchesArray.includes('all')) {
      ...
      <div className="page-container" style={{ padding: 0 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '15px' }}>
          <div>
            ...
            <p style={{ color: 'var(--text-muted)', margin: 0 }}>Daily academic logs and Technical Lead reviews.</p>
          </div>
          {isTrainer && (
        ...
        <div className="table-container">
          <table className="modern-table" style={{ whiteSpace: 'nowrap' }}>
              ...
              <tr>
                ...
                {isTL && (
                  <>
                    ...
                    <th style={{ textAlign: 'center', background: 'rgba(245, 158, 11, 0.1)' }}>Review</th>
                  </>
                )}
              </tr>
            </thead>
            <tbody>
              {loading ? (
                <tr><td colSpan={isTL ? 6 : 3} style={{ textAlign: 'center', padding: '3rem' }}><CircleNotch size={32} className="ph-spin" color="#38bdf8" /></td></tr>
              ) : logs.length === 0 ? (
                <tr><td colSpan={isTL ? 6 : 3} style={{ textAlign: 'center', padding: '3rem', color: 'var(--text-muted)' }}>No logs submitted yet.</td></tr>
              ) : (
                logs.map((log, i) => (
                  <tr key={i}>
                    <td>
                      <span className="primary-text">{log.timestamp.split(' ')[0]}</span>
                      <span className="sub-text">{log.trainerName} • {log.branch}</span>
                    </td>
                    ...
                    <td style={{ color: '#cbd5e1', whiteSpace: 'normal', minWidth: '200px' }}>{log.feedbacks || '-'}</td>
