...
export default function TechnicalExams() {
  ...
  const getSubCourses = (mainCourse) => {
    ...
    const dictKey = Object.keys(courseDict).find(k => k.toLowerCase().replace(/[^a-z0-9]/g, '') === cleanMain);
    if (dictKey && courseDict[dictKey].length > 0) return courseDict[dictKey];
    return [];
  };
  const subCoursesList = getSubCourses(selectedMainCourse);
          ...
          <>
            ...
            {subCoursesList.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '3rem', background: 'var(--card-bg)', borderRadius: '12px', border: '1px solid var(--card-border)' }}>
                <p style={{ color: 'var(--text-muted)' }}>No programs found in this domain.</p>
              </div>
            ) : (
          ...
          <>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '25px', flexWrap: 'wrap', gap: '15px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                <button 
                  onClick={() => { setViewLevel('sub_courses'); setSelectedSubCourse(null); }} 
                  style={{ background: 'var(--card-bg)', border: '1px solid var(--card-border)', color: '#fff', padding: '10px 15px', borderRadius: '8px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '5px' }}
                >
            ...
            <div style={{ display: 'flex', gap: '10px', marginBottom: '20px', borderBottom: '1px solid var(--card-border)', paddingBottom: '10px' }}>
              ...
              <button 
                ...
                style={{ background: activeTab === 'results' ? 'rgba(16, 185, 129, 0.1)' : 'transparent', color: activeTab === 'results' ? '#10b981' : 'var(--text-muted)', border: 'none', padding: '10px 20px', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '8px' }}
              >
                      ...
                      filteredQuestions.map((q, i) => (
                        <tr key={i}>
                          <td style={{ verticalAlign: 'top', padding: '16px' }}>
                            <span className="primary-text" style={{ whiteSpace: 'normal', lineHeight: 1.5, display: 'block', marginBottom: '6px' }}>
                              {q.question}
                            </span>
                              ...
                              {canManage && (
                                <>
                                  <button 
                                    ...
                                    style={{ background: 'rgba(245, 158, 11, 0.1)', color: '#f59e0b', border: '1px solid #f59e0b', padding: '8px', borderRadius: '8px', cursor: 'pointer' }} 
                                    title="Edit"
                                  >
          ...
          <div className="modal-card" style={{ maxWidth: '700px', width: '100%', maxHeight: '90vh', overflowY: 'auto', background: '#0f1523', border: '1px solid var(--card-border)', borderRadius: '16px', padding: '2rem' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #1e293b', paddingBottom: '1rem', marginBottom: '1.5rem' }}>
              <h2 style={{ margin: 0, display: 'flex', alignItems: 'center', gap: '10px' }}>
                <ListChecks color="var(--accent-primary)" /> {isEditMode ? 'Edit Exam Question' : 'Add Exam Question'}
              </h2>
            ...
            <form onSubmit={handleSubmit}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '15px', marginBottom: '15px' }}>
                <div className="form-group">
                  ...
                  <input type="text" name="id" value={formData.id} readOnly style={{ background: 'var(--bg-dark)', opacity: 0.7 }} className="sleek-input" />
                </div>
                <div className="form-group">
              ...
              <div className="form-group" style={{ marginBottom: '15px' }}>
                ...
                <textarea name="question" className="sleek-input" style={{ width: '100%', minHeight: '80px' }} value={formData.question} onChange={handleInputChange} required />
              </div>

              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px', marginBottom: '15px' }}>
                <div className="form-group">
                  <label>Option A</label>
                  <input type="text" name="optA" value={formData.optA} onChange={handleInputChange} required className="sleek-input" />
                </div>
                <div className="form-group">
                  <label>Option B</label>
                  <input type="text" name="optB" value={formData.optB} onChange={handleInputChange} required className="sleek-input" />
                </div>
                <div className="form-group">
                  <label>Option C</label>
                  <input type="text" name="optC" value={formData.optC} onChange={handleInputChange} required className="sleek-input" />
                </div>
                <div className="form-group">
                  <label>Option D</label>
                  <input type="text" name="optD" value={formData.optD} onChange={handleInputChange} required className="sleek-input" />
                </div>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '15px', marginBottom: '20px' }}>
                ...
                <div className="form-group">
                  ...
                  <input type="text" name="explanation" value={formData.explanation} onChange={handleInputChange} className="sleek-input" style={{ width: '100%' }} />
                </div>
              </div>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px', borderTop: '1px solid #1e293b', paddingTop: '1.5rem' }}>
                ...
                <button type="submit" className="btn-action" style={{ width: 'auto' }} disabled={isSubmitting}>
                  {isSubmitting ? <CircleNotch size={20} className="ph-spin" /> : (isEditMode ? 'Save Changes' : 'Save Question')}
                </button>
              </div>
            </form>
