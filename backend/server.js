...
async function refreshCache() {
  try {
    ...
    const [stuSheet, appSheet, vacSheet, eventSheet, issueSheet, tSchedSheet, tAttSheet, clientSheet] = [
      getSheet("Data"), 
      getSheet("Opening_Applied"), 
      getSheet("NewsLetter"),
      getSheet("Event"), 
      getSheet("Issues"), 
      getSheet("Talentino_Schedule"), 
      getSheet("Talentino_Attendance"), 
      getSheet("Clients") 
    ];

    const [stuRows, appRows, vacRows, eventRows, issueRows, tSchedRows, tAttRows, clientRows] = await Promise.all([
      stuSheet ? stuSheet.getRows() : [], 
      appSheet ? appSheet.getRows() : [], 
      vacSheet ? vacSheet.getRows() : [],
      eventSheet ? eventSheet.getRows() : [], 
      issueSheet ? issueSheet.getRows() : [], 
      tSchedSheet ? tSchedSheet.getRows() : [], 
      tAttSheet ? tAttSheet.getRows() : [], 
      clientSheet ? clientSheet.getRows() : []
    ]);
...
app.use('/api/tpo', (req, res, next) => {
  if (!globalCache) {
    return res.status(503).json({ success: false, message: "Server is warming up, please try again in 5 seconds." });
  }
  next();
});
...
app.post('/api/auth/login', async (req, res) => {
  ...
  try {
    ...
    const tpoRow = rows.find(row => 
      (row.get('Mail ID') || '').toString().trim().toLowerCase() === cleanEmail && 
      (row.get('Password') || '').toString().trim() === password
    );
...
app.post('/api/tpo/dashboard-stats', (req, res) => {
  ...
  globalCache.students.forEach(row => { 
    if (checkBranchMatch(row.get('Branch'), assignedBranchesArray)) studentCount++; 
  });
  ...
  let eventsList = globalCache.events.slice(-8).map(row => ({
    ...
    date: row.get('Date') || '', 
    time: row.get('Time') || '', 
    type: row.get('Type') || 'Placement Drive', 
    location: row.get('Location') || ''
  }));

  res.json({ 
    success: true, 
    stats: { totalStudents: studentCount, pendingApps, placed: placedCount, activeVacancies: activeVacs }, 
  ...
  globalCache.students.forEach(row => {
    ...
    if (checkBranchMatch(branch, assignedBranchesArray)) {
      ...
      students.push({
        ...
        name: rowData['Name'] || '', 
        email: rowData['Mail ID'] || '', 
        phone: phone,
        roll: rowData['IPCS Roll Number'] || rowData['Roll Number'] || '', 
        branch: branch, 
        course: course,
        photo: rowData['Profile Photo'] || '', 
        qual: rowData['Qualification'] || '', 
  ...
  globalCache.applications.forEach((row) => {
    ...
    if (checkBranchMatch(branch, assignedBranchesArray) || (cleanTpoName !== '' && officerName === cleanTpoName)) {
      ...
      apps.push({
        ...
        roll: roll, 
        branch: branch, 
        course: rowData[getHeader('course')] || '', 
        qual: qual || 'Not Specified', 
        jobId: rowData[getHeader('job id')] || '', 
        company: rowData[getHeader('company')] || 'Unknown Company', 
        position: rowData[getHeader('position')] || 'Unknown Position',
        date: rowData[getHeader('time')] || rowData[getHeader('date')] || '', 
        status: rowData[getHeader('status')] || 'Applied', 
        remarks: rowData[getHeader('remarks')] || '', 
        tpoName: rowData[getHeader('placement officer')] || '',
        phone: phone, 
        email: email, 
        resume: resume,
    ...
    if (rows.length > 0) {
      ...
      if (logSheet && fullApp) {
        await logSheet.addRow({
          ...
          'Student Name': fullApp.name || '', 
          'Contact': fullApp.phone || '', 
          'Mail ID': fullApp.email || '',
          'Roll Number': fullApp.roll || '', 
          'Course': fullApp.course || '', 
          'Branch': fullApp.branch || '',
          'Qualification': fullApp.qual || '', 
          'Resume': fullApp.resume || '', 
          'Job ID': fullApp.jobId || '',
          'Company Name': fullApp.company || '', 
          'Placement Officer': fullApp.tpoName || '', 
          'Status': status || '', 
          'Remarks': remarks || '',
...
app.post('/api/tpo/applications/add', upload.single('offerLetterFile'), async (req, res) => {
  ...
  try {
    ...
    const newRow = {
      'TimeStamp': new Date().toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' }),
      'Student Name': appData.name, 
      'Contact': appData.phone, 
      'Mail ID': appData.email, 
      'Roll Number': appData.roll,
      'Course': appData.course, 
      'Branch': appData.branch, 
      'Qualification': appData.qual || '', 
      'Resume': appData.resume || '',
      'Job ID': 'MANUAL-ADD', 
      'Company Name': appData.company, 
      'Position': appData.position,
      'Placement Officer': tpoName, 
      'Status': appData.status || 'Placed', 
      'Remarks': appData.remarks,
      'DATE PLACED': appData.datePlaced, 
      'PACKAGE (LPA)': appData.packageLpa,
...
app.get('/api/tpo/vacancies', (req, res) => {
  let vacs = globalCache.vacancies.map((row, i) => ({
    id: row.get('Job ID') || row.get('ID') || `JOB-${i+1}`, 
    company: row.get('Company') || '', 
    position: row.get('Position') || row.get('Role') || '', 
    location: row.get('Location') || '', 
    state: row.get('State') || 'OTHER', 
    mode: row.get('Mode') || '', 
    lastDate: row.get('Last Date') || '', 
    status: row.get('Status') || 'Open', 
    course: row.get('Course') || 'All'
  }));
...
app.get('/api/tpo/events', (req, res) => {
  let allEvents = globalCache.events.map(row => ({
    date: row.get('Date') || '', 
    branch: row.get('Branch') || 'Bangalore', 
    title: row.get('Title') || 'Placement Drive', 
    type: row.get('Type') || 'Placement Drive', 
    time: row.get('Time') || '', 
    location: row.get('Location') || ''
  }));
...
app.post('/api/tpo/reports', (req, res) => {
  ...
  globalCache.students.forEach(row => { 
    if (checkBranchMatch(row.get('Branch'), assignedBranchesArray)) {
      students.push({ name: row.get('Name'), roll: row.get('Roll Number'), branch: row.get('Branch'), course: row.get('Course'), status: row.get('Status') }); 
    }
  });
  
  globalCache.applications.forEach(row => { 
    if (checkBranchMatch(row.get('Branch'), assignedBranchesArray)) {
      applications.push({ name: row.get('Student Name'), roll: row.get('Roll Number'), jobId: row.get('Job ID'), company: row.get('Company Name'), date: row.get('TimeStamp'), status: row.get('Status'), remarks: row.get('Remarks') }); 
    }
  });
  
  globalCache.issues.forEach(row => { 
    if (checkBranchMatch(row.get('Branch'), assignedBranchesArray)) {
      issues.push({ name: row.get('Name'), branch: row.get('Branch'), details: row.get('Issue Details'), status: row.get('Status'), remarks: row.get('Remarks') }); 
    }
  });
  
  globalCache.tAtt.forEach(row => { 
    if (checkBranchMatch(row.get('Branch'), assignedBranchesArray)) {
      talentino.push({ name: row.get('Name'), branch: row.get('Branch'), date: row.get('Check-in') || row.get('Date'), rating: row.get('Rating'), notes: row.get('Notes') }); 
    }
  });
  
  let vacancies = globalCache.vacancies.map(row => ({ 
    id: row.get('Job ID') || row.get('ID') || '', 
    company: row.get('Company') || '', 
    location: row.get('Location') || '', 
    mode: row.get('Mode') || '', 
    status: row.get('Status') || 'Open' 
...
app.post('/api/tpo/talentino', (req, res) => {
  ...
  globalCache.tSched.forEach(row => { 
    const d = row.get('Date'); 
  ...
  let records = globalCache.tAtt.filter(row => checkBranchMatch(row.get('Branch'), assignedBranchesArray)).map(row => ({
    name: row.get('Name') || '', 
    branch: row.get('Branch'), 
    date: row.get('Check-in') || row.get('Date') || '', 
      ...
      html: `
        <div style="font-family: Arial, sans-serif; color: #333; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 8px; overflow: hidden;">
          <div style="background-color: #0f1523; padding: 20px; text-align: center; border-bottom: 4px solid #38bdf8;">
            <h2 style="color: #ffffff; margin: 0;">IPCS PARTNERSHIP</h2>
          </div>
          <div style="padding: 30px;">
            <p>Dear ${companyName} Team,</p>
            <p>We are thrilled to welcome you as a Preferred Hiring Partner with IPCS Global!</p>
            <p>To finalize our association, please review and digitally sign your Certificate of Partnership by clicking the secure button below. You will be able to upload your company logo and authorized signature directly on the document.</p>
            <div style="text-align: center; margin: 40px 0;">
              <a href="${signingLink}" style="background-color: #10b981; color: white; padding: 14px 28px; text-decoration: none; font-weight: bold; border-radius: 6px; font-size: 16px;">Review & Sign Certificate</a>
            </div>
            <p style="font-size: 13px; color: #64748b;">If the button does not work, copy and paste this link into your browser: <br/>${signingLink}</p>
          </div>
        </div>
      `
    };
